 ...
 * @version 1.0
 *
 * Driver containing the master functions that can be used to control the OWI - Temperature Humidity Sensor.
 */
 ...
 */
class OWI_TH_Sensor : public OWI::Device {
public:
	/** Family code. */
	 ...
	 */
	explicit OWI_TH_Sensor(OWI& owi, uint8_t* rom = NULL) :
		OWI::Device(owi, rom),
		m_label(0)
	{}
	 ...
	 * @return zero(0) or negative error code.
	 */
	int sensorReadScrachpad(double* temperature, double* humidity, double* dewpoint) {
		if (!MATCH()) return -1;
		...
		// check validity of response variables and return the values to caller
			if(!isnan(m_sensor_scratchpad.temperature) && !isnan(m_sensor_scratchpad.relative_humidity) && !isnan(m_sensor_scratchpad.dewpoint)) {
				*temperature = m_sensor_scratchpad.temperature;
				*humidity = m_sensor_scratchpad.relative_humidity;
				*dewpoint = m_sensor_scratchpad.dewpoint;
				return 0;
			}
		}
		return -1;
