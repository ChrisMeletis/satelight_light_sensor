 ...
 */
class OWI_PH_Sensor : public OWI::Device {
public:
	/** Family code. */
	 ...
	 */
	explicit OWI_PH_Sensor(OWI& owi, uint8_t* rom = NULL) :
		OWI::Device(owi, rom),
		m_label(0)
	{}
	 ...
	 * @return zero(0) or negative error code.
	 */
	int sensorReadScrachpad(uint16_t* lux){
		if (!MATCH()) return (-1);
		...
		if(m_owi.read(&m_sensor_scratchpad, sizeof(m_sensor_scratchpad))){	//read response
			...
			if(!isnan(m_sensor_scratchpad.lux)) {
				*lux = m_sensor_scratchpad.lux;
				return(0);
			}
		}
		return(-1);
